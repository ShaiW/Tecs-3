...
package simulators.CPUEmulator;

import java.util.*;

import simulators.ComputerParts.*;
import simulators.Events.*;
import simulators.GUI.MemoryComponent;
import simulators.GUI.components.ROMComponent;
...
public class ROM extends PointedMemory implements ProgramEventListener
{
    ...
    public ROM(ROMGUI gui) {
        ...
        setNullValue(AssemblyTranslator.NOP, true);
...
//          gui.setNumericFormat(BINARY_FORMAT); // disable assembler
    ...
    public synchronized void loadProgram(String fileName) throws ProgramException {
        ...
        try {
            contents = AssemblyTranslator.loadProgram(fileName);

            if (displayChanges) {
                ((ROMComponent)gui).setContents(contents);
    ...
    public void valueChanged(ComputerPartEvent event) {
        ...
        clearErrorListeners();
        try {
            AssemblyTranslator.getInstance().codeToText(newValue);

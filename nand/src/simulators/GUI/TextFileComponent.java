...
public class TextFileComponent extends JPanel implements TextFileGUI {
	...
	public void addLine(AsmLine line) {
		rowsVector.addElement(Conversions.decimalToBinary(line.value(), 16));
		if (line.comment() != null) 
			comments.put(rowsVector.size()-1, line.comment());
		if (line.label() != null){
			labels.put(rowsVector.size()-1, line.label());		
			labelList.addLabel(line.label(), rowsVector.size()-1);
		}
		textFileTable.revalidate();
        repaint();
	}
    ...
    public void setContents (AsmProgram program) {
        ...
        for (AsmProgram.AsmLine l : program) {
    		...
    		if (l.comment() != null) 
    			comments.put(rowsVector.size()-1, l.comment());
    		if (l.label() != null){
    			labels.put(rowsVector.size()-1, l.label());
    		}
    ...
    public class TextFileCellRenderer extends DefaultTableCellRenderer {
        ...
        public void setRenderer(int row, int column) {
            ...
            if (labels.containsKey(row))
            	setBackground(Color.green);
            
            setToolTipText((labels.containsKey(row) ? "("+labels.get(row)+")" : "") + 
            		((labels.containsKey(row) && (comments.containsKey(row)) ? "     " : "") +
            		(comments.containsKey(row) ? comments.get(row) : "")));
        }
    }
}

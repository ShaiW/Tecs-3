...
import simulators.Events.*;
import simulators.GUI.*;
import translators.assembler.AsmProgram;
import translators.assembler.AssemblerException;
import translators.assembler.AssemblyTranslator;
import translators.assembler.AsmProgram.AsmLine;
...
public class ROMComponent extends PointedMemoryComponent implements ROMGUI {
    ...
    private AssemblyTranslator translator = AssemblyTranslator.getInstance();
    ...
    public String getValueStr (short address) {
        return translateValueToString(contents.getLine((int) address).value());
    }
    ...
    public short getValueAsShort (short address) {
        return contents.getLine((int) address).value();
    }
    ...
    public class ROMTableCellRenderer extends PointedMemoryTableCellRenderer {
        ...
        public void setRenderer(int row, int column) {
        	if(column == 0) setHorizontalAlignment(SwingConstants.CENTER);
            	...
            	if(dataFormat==ASM_FORMAT) setHorizontalAlignment(SwingConstants.LEFT);
            	else setHorizontalAlignment(SwingConstants.RIGHT);
            }
        	if (row>contents.size()) return;
        	AsmLine current = contents.getLine(row);
        	
        	/****************/
        	
        	setToolTipText("");
        	if (current.label() != null) setToolTipText("("+current.label()+")");
        	if (current.comment() != null) setToolTipText(getToolTipText() + "\t" + current.comment());
        	if (getToolTipText().isEmpty()) setToolTipText(null);
        	else setBackground(Color.GREEN);
        	 if (row == pointerAddress) setBackground(Color.yellow);
        	/****************/
        }
    }
    
    class ROMTableModel extends AbstractTableModel{
		...
		public Object getValueAt(int row, int column) {
			if (column == 0) return addresses[row];
			else {
				if (row<contents.size()) {
					if (dataFormat == ASM_FORMAT)
						try {
							return AssemblyTranslator.getInstance().codeToText(contents.getLine(row).value());
						} catch (AssemblerException e) {
							assert(false);
						}
					return Format.translateValueToString(contents.getLine(row).value(), dataFormat);
				}
				
				else return AsmProgram.NULL_VALUE;
			}
		}
    ...
    protected TableModel getTableModel() {
        return new ROMTableModel();
    }

	public void setContents(AsmProgram contents) {

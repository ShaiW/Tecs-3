...
public abstract class HackTranslator implements HackTranslatorEventListener, ActionListener,
     ...
     */
    public HackTranslator(String fileName, boolean save)
     ...
     */
    public HackTranslator(HackTranslatorGUI gui, String sourceFileName)
     throws HackTranslatorException {
        this.gui = gui;
    ...
    protected void addCommand(short command) throws HackTranslatorException {
        ...
        program.setLine(destPC++, command, null, null); //TODO
        if (updateGUI)
            ((TextFileComponent) gui.getDestination()).addLine(getCodeString(command, destPC - 1, true));
    }
    ...
    protected void replaceCommand(int pc, short command) {
    	program.setLine(pc, command, null, null);
        if (updateGUI)
    ...
    private void dumpToFile() {
        for (short i = 0; i < programSize; i++)
            writer.println(getCodeString(program.getLine(i).value(), i, false));
        writer.close();
    }

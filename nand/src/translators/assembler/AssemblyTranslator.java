...
import simulators.GUI.components.ProgramComponent;
import translators.*;
...
public class AssemblyTranslator {
	...
	// the single instance
	private static AssemblyTranslator instance;
	 ...
	 */
	public static AssemblyTranslator getInstance() {
		if (instance == null)
			new AssemblyTranslator();
		return instance;
	}
	...
	public String codeToText(short code) throws AssemblerException {
		...
		if (code != AssemblyTranslator.NOP) {
			if ((code & 0x8000) == 0) {
				command.append('@');
				command.append(code);
			}
	...
	public static AsmProgram loadProgram(String fileName)
	throws AssemblerException {

		File file = new File(fileName);
		AsmProgram prog = new AsmProgram(Definitions.getInstance().getAddressesTable());
		if (!file.exists())
		...
		if (fileName.endsWith(".hack")) {
			try {
				...
				while ((line = reader.readLine()) != null) {
					...
					if (pc >= AsmProgram.MAXIMUM_PROGRAM_SIZE)
			...
			try {
				AssemblyParser assembler = new AssemblyParser(fileName, false);
	...
	//    public static short[] loadProgram(String fileName)
	//     throws AssemblerException {
	//        short[] memory = null;
	//
	//        File file = new File(fileName);
	...
	//
	//        if (fileName.endsWith(".hack")) {
	//            memory = new short[AsmProgram.MAXIMUM_PROGRAM_SIZE];

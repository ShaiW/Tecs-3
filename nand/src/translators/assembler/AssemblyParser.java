...
import simulators.GUI.TextFileComponent;
import translators.*;
 ...
 */
public class AssemblyParser extends HackTranslator {
    ...
    private AssemblyTranslator translator;
     ...
     */
    public AssemblyParser(String fileName, boolean save)
     ...
     */
    public AssemblyParser(HackAssemblerGUI gui, int size, short nullValue, String sourceFileName)
     throws HackTranslatorException {
        super(gui, sourceFileName);
    ...
    protected void init() {
        super.init();
        translator = AssemblyTranslator.getInstance();
    }
    ...
    private BidiMap<String, Short> generateSymbolTable() throws HackTranslatorException {
    	BidiMap<String, Short> symbolTable = Definitions.getInstance().getAddressesTable();
        short pc = 0;
        String line;
        String label;
...
//            result = (program.getLine(compiledRange[0] + i).value() == comparisonProgram[compiledRange[0] + i]);
//
//        return result;
//    }
        ...
        try {
            ...
            if (!input.isEnd() && !input.isToken("(")) {
                if (input.isToken("@")) {
                    ...
                    if (!numeric) {
                    	if (program.addLabel((short)(varIndex+1), label)) varIndex++;
                        addCommand(translator.textToCode("@" + program.getSymbols().get(label)), input.comment());
                    }
                    else
                        addCommand(translator.textToCode(line), input.comment());
                }
    ...
    public void actionPerformed(HackTranslatorEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case AssemblerEvent.COMPARISON_LOAD:
                clearMessage();
    ...
    protected void addCommand(short command, String comment) throws HackTranslatorException {
        if (destPC++ >= AsmProgram.MAXIMUM_PROGRAM_SIZE)
            throw new HackTranslatorException("Program too large");
        program.addLine(command, comment);
        if (updateGUI && command != AsmProgram.NULL_VALUE)
            ((TextFileComponent) gui.getDestination()).addLine(program.lastLine());
    }
}
